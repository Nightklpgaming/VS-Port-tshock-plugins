...
[ApiVersion(2, 1)]
public class MainRB : TerrariaPlugin
{
    public override string Name => "RecipesBrowser";

    public override Version Version => new Version(1, 0, 6);

    public override string Author => "Prism Feather Adaptation";

    public override string Description => "Obtain the item synthesis table through commands";

    public MainRB(Main game)
        : base(game)
    {
    }
    ...
    {
        Commands.ChatCommands.Add(new Command("", this.FindRecipe, "check", "find", "fd"));
    ...
    private void FindRecipe(CommandArgs args)
    {
        ...
        //Check if the player has the "Check synthesis table" permission
        if (!args.Player.HasPermission("RecipesBrowser"))
        {
            // If there is no permission, send an error message to the player and exit the method
            args.Player.SendErrorMessage("You do not have permission to use the crafting table command");
            return;
        }
        ...
        {
            args.Player.SendInfoMessage("Multiple item matches retrieved");
            var text = "";
        ...
        var text3 = $"thing:{Lang.GetItemNameValue(item.type)}[i:{item.type}]\n";
        if (list.Count > 1)
        {
        ...
        else
        {
            if (list.Count != 1)
            {
                args.Player.SendErrorMessage("This item has no recipe");
                return;
            }
    ...
    private string GetRecipeStringByResult(Recipe recipe)
    {
        ...
        text += "\n";
        text += "Required synthesis station：";
    ...
    {
        var text = "Items that can be crafted:\n";
